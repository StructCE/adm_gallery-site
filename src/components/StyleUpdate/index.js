...
const StyleUpdate = ({showModal, setShowModal}) => {
    ...
    const loadStyle = async () => {
        const response = await api.get(`/api/v1/styles/show/${id}`)
    ...
    const handleSubmit = async (e) => {
        ...
        try{
            const response = await api.post(`/api/v1/styles/update/${id}`, {
                style: {
                    title,
                    description
                }
        ...
        <Modal showModal={showModal} setShowModal={setShowModal}>
            <Container>
                <form onSubmit={handleSubmit}>
                    ...
                    />
                    <FormsButton type="submit">Editar Estilo</FormsButton>
                </form>
            </Container>
        </Modal>
    )
